...
cells.append(new_markdown_cell(
    "## 0단계 · 환경 점검\n"
    "\n"
    "본 챕터에서 새로 사용할 라이브러리는 pandas 하나다.\n"
    "\n"
    "- **pandas** — 표(DataFrame) 자료 처리. CSV·JSON·Excel 입출력과 필터·집계의 표준 도구.\n"
    "\n"
    "**설치 — VS Code 통합 터미널에서**\n"
    "\n"
    ...
    "   ```\n"
    "   → `OK 2.x.x` 같은 줄이 나오면 성공. `.venv` 안에만 설치되어 시스템 Python에는 영향 없음."
))
...
cells.append(new_code_cell(
    ...
    "    {'field':'embedding_flag','type':'boolean','required':False,\n"
...
cells.append(new_markdown_cell(
    ...
    "\n"
    "이 단계에서 `summary` · `keywords` · `chunk_ids`는 비워 두고 `embedding_flag`는 False로 둔다.\n"
...
cells.append(new_code_cell(
    ...
    "     'license_code':'ALL-RIGHTS-RESERVED','chunk_ids':[],'embedding_flag':False},\n"
    "\n"
    ...
    "     'license_code':'CC-BY-4.0','chunk_ids':[],'embedding_flag':False},\n"
    "\n"
    ...
    "     'license_code':'KOGL-1','chunk_ids':[],'embedding_flag':False},\n"
    "\n"
    ...
    "     'license_code':'CC-BY-SA-3.0','chunk_ids':[],'embedding_flag':False},\n"
    "])\n"
...
cells.append(new_markdown_cell(
    ...
    "- Ch.6 §6.5 : 벡터DB(ChromaDB) 적재 → `embedding_flag` True로 켜기\n"
