# -*- coding: utf-8 -*-
r"""
ch03_collected.jsonl 생성기

arXiv API에서 RAG 관련 논문 5건을 가져와 Ch.2 §2.4 스키마에 매핑한다.
...
def main():
    ...
    df.to_json("ch03_collected.jsonl", orient="records", lines=True, force_ascii=False)
    print(f"\n[OK] ch03_collected.jsonl ({len(df)} records)")
    ...
    try:
        ...
        print(f"\n[Ch.2 스키마 검증] {'통과' if not errors else errors}")
