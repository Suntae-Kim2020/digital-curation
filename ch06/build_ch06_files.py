...
- ch06_collected_embedded.jsonl
    Ch.3 스키마의 마지막 필드 embedding_flag까지 채운 완성형.
    embedding_flag는 임베딩 처리 완료 여부를 표시하는 boolean이다 (True/False).
...
def main():
    ...
    # embedding_flag = True (자료의 청크가 ChromaDB에 적재됐다는 신호)
    filled_count = 0
    ...
    print(f"  → embedding_flag=True 처리된 레코드: {filled_count}")
