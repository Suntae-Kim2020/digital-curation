# -*- coding: utf-8 -*-
r"""
ch02_metadata_schema.ipynb 생성기 — Colab에서 바로 열 수 있는 노트북

교재 Chapter 2 §2.4를 그대로 따른다.
"""
import io
import sys
...
cells.append(new_markdown_cell(
    "# Chapter 2 §2.4 실습 — 메타데이터 스키마 만들기\n"
    "\n"
    "> 『AI 레디 데이터와 디지털 큐레이션』 Chapter 2의 §2.4 실습 노트북입니다.\n"
    ...
    "## 산출물\n"
    "\n"
    "1. `ch02_schema.csv` — 22개 필드 스키마\n"
    "2. `ch02_sample.jsonl` — 샘플 레코드 5건\n"
    "3. 검증 결과 '통과'\n"
    "\n"
    "## 선행 학습\n"
    "\n"
    "- Ch.1 §1.3 AI 레디 데이터 5조건\n"
    "- 본 챕터 §2.2 Dublin Core 15요소 / §2.3 AI 확장 6필드\n"
))
...
cells.append(new_code_cell(
    ...
    "    # ── AI 활용 확장 ──\n"
    "    {'field':'summary',      'type':'string',   'required':False, 'category':'AI', 'desc':'LLM 요약 (Ch.6 §6.4)'},\n"
    "    {'field':'keywords',     'type':'string[]', 'required':False, 'category':'AI', 'desc':'추출 키워드 (Ch.4 §4.3)'},\n"
    ...
    "    {'field':'chunk_ids',    'type':'string[]', 'required':False, 'category':'AI', 'desc':'청크 ID (Ch.4 §4.4)'},\n"
    "    {'field':'embedding_id', 'type':'string',   'required':False, 'category':'AI', 'desc':'벡터DB ID (Ch.5 §5.3)'},\n"
    "])\n"
    "schema.head()"
))
...
cells.append(new_code_cell(
    "schema.to_csv('ch02_schema.csv', index=False, encoding='utf-8-sig')\n"
    "print('Saved:', 'ch02_schema.csv', '·', len(schema), 'fields')"
))
...
cells.append(new_markdown_cell(
    ...
    "Ch.4 §4.3에서 키워드를, Ch.4 §4.4에서 청크 ID를, Ch.5 §5.3에서 임베딩 ID를, Ch.6 §6.4에서 요약을 단계별로 채울 것이다."
))
...
cells.append(new_code_cell(
    "sample.to_json('ch02_sample.jsonl', orient='records', lines=True, force_ascii=False)\n"
    "print('Saved:', 'ch02_sample.jsonl', '·', len(sample), 'records')\n"
    "\n"
    "# 첫 줄 확인\n"
    "with open('ch02_sample.jsonl', 'r', encoding='utf-8') as f:\n"
    "    print('\\n[첫 줄 미리보기]')\n"
...
cells.append(new_markdown_cell(
    ...
    "\n"
    "더 엄밀한 검증(자료형 일치, 코드값 표준 일치)은 Ch.4 §4.1 데이터 정제에서 다룬다."
))
...
cells.append(new_markdown_cell(
    "## 다음 단계\n"
    "\n"
    "- Ch.3 §3.1~§3.4 : 외부 API/웹에서 자료를 실제로 수집해 이 스키마에 매핑\n"
    "- Ch.4 §4.3 : 본문에서 `keywords` 자동 추출 (Kiwi 형태소 분석)\n"
    "- Ch.4 §4.4 : 본문 청킹 → `chunk_ids` 채우기\n"
    "- Ch.5 §5.3 : 벡터DB(ChromaDB) 적재 → `embedding_id` 채우기\n"
...
with open("ch02_metadata_schema.ipynb", "w", encoding="utf-8") as f:
    nbf.write(nb, f)

print("[OK] ch02_metadata_schema.ipynb", f"({len(cells)} cells)")
