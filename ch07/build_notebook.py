...
cells.append(new_markdown_cell(
    ...
    "\n"
    "> **`thinking_config`가 왜 필요한가** — `gemini-2.5-flash`는 답하기 전에 속으로 "
    "*생각(thinking)* 하는 모델이다. 이 생각도 토큰을 소비하는데, 그 양이 `max_output_tokens` "
    "한도 **안에** 포함된다. 그래서 한도를 작게 잡으면 생각이 한도를 다 써 버려 정작 답변이 "
    "잘리거나 빈 문자열이 나온다(특히 JSON 응답에서 `Unterminated string` 오류로 드러난다). "
    "요약·추출처럼 추론이 필요 없는 작업에서는 `thinking_config={'thinking_budget': 0}`으로 "
    "생각을 꺼 두면 토큰 전부가 답변에 쓰여 빠르고 저렴하고 결과도 안정적이다."
))
...
cells.append(new_code_cell(
    ...
    "\n"
    "all_full = all(\n"
    "    sum(1 for v in complete[f] if (isinstance(v, list) and v) or (isinstance(v, str) and v.strip()) or (isinstance(v, bool) and v)) == len(complete)\n"
    "    for f in ai_fields\n"
    ")\n"
    "print()\n"
